    ...
    return (
        <ScrollView style = {{ "backgroundColor": colours.colour_2 }}>
            <View className = "items-center m-[5px]">
                    ...
                    {fields.map((field, index) => { return (
                        <View key = {field.id} className = "flex-initial flex-col w-full justify-between mt-[15px] flex-wrap items-center">
                            <View className = "flex-row w-full">
                                <View className = {`bg-[${colours.colour_6}] items-center min-h-[100px] flex-1 m-2.5 p-{20px}`}>
                                ...
                                <View className = {`bg-[${colours.colour_6}] items-center min-h-[100px] flex-1 m-2.5 p-{20px}`}>
                                    ...
                                    <Text className = {`text-center w-11/12 flex-1 m-2.5 bg-[${colours.colour_5}] h-[27px] leading-[35px]`}>N/A</Text>
                                </View>
                                <View className = {`bg-[${colours.colour_6}] items-center min-h-[100px] flex-1 m-2.5 p-{20px}`}>
                                    ...
                                    <Controller
                                        ...
                                        className = {`align-middle text-center w-11/12 flex-1 m-2.5 bg-[${colours.colour_5}] h-5`}
                                        render = {({ "field": { onChange, onBlur, value } }) => { return (
                                            <TextInput
                                                onBlur = {onBlur}
                                                ...
                                                className = {`align-middle text-center w-11/12 flex-1 m-2.5 bg-[${colours.colour_5}]`}
                                            />
                                        ); }}
                                    />
                                </View>
                            ...
                            {!removedButtons.includes(index) && (
                                <TouchableOpacity onPress = {() => {
                                    updateData();
                                    addSet(index);
                                }} className = {`mt-[100px] bg-[${colours.colour_12}] p-2 m-[5px]`}>
                ...
                <TouchableOpacity onPress = {handleSubmit(onSubmit)} className = {`mt-[100px] bg-[${colours.colour_12}] p-2 m-[5px]`}>

...
const WorkoutForm = ({ saveTo, defaultValues }) => {
    ...
    const { fields, append, insert, remove } = useFieldArray({ control, "name": "exercises", "keyName": "id" });
    ...
    const names2 = names.map((name) => { return { "label": name, "value": name }});
    ...
    const trimWorkoutData = (data) => {
        return { ...data, "workoutName": data.workoutName?.trim(), "workoutNotes": data.workoutNotes?.trim(), "exercises": data.exercises?.map((exercise) => { return {
                ...exercise,
        ...
        const loadSavedData = async () => {
            ...
            if (id) {
                if (source === "workout-history") {
                    ...
                    const exercisesData = exerciseRecords.map((exercise) => { return { "name": exercise.exercises.name, "duration": exercise.metrics, "reps": exercise.volume, "personalBest": "N/A" }; });
                    reset({ "workoutName": workout.name, "workoutNotes": workout.notes, "exercises": exercisesData });
                    ...
                    const exercisesData = exerciseRecords.map((exercise) => { return { "name": exercise.exercises.name, "duration": exercise.metrics, "reps": exercise.volume, "personalBest": "N/A" }; });
                    reset({ "workoutName": workoutPreset.name, "workoutNotes": workoutPreset.notes, "exercises": exercisesData });
                }
            } else {
                const savedData = await loadFormData();
                if (savedData) {
                    reset(savedData);
                    if (savedData.workoutDate) { setWorkoutDate(new Date(savedData.workoutDate)); }
                }
            }
        };
            ...
            if (id) {
                realmInstance.write(() => {
                    if (source === "workout-history") {
                        ...
                        trimmedData.exercises.forEach((exercise) => {
                            ...
                            realmInstance.create("PreviousWorkoutsExercises", { "id": newExerciseId, "previousWorkouts": existingWorkout, "exercises": exerciseObj, "metrics": exercise.duration, "volume": exercise.reps.toString() });
                        });
                    } else if (source === "workout-presets") {
                        ...
                        const existingExercises = realmInstance.objects("WorkoutPresetsExercises").filtered("workoutPresets.id == $0", parseInt(id));
                        realmInstance.delete(existingExercises);
                        ...
                        trimmedData.exercises.forEach((exercise) => {
                            ...
                            realmInstance.create("WorkoutPresetsExercises", { "id": newExerciseId, "workoutPresets": existingPreset, "exercises": exerciseObj, "metrics": exercise.duration, "volume": exercise.reps.toString() });
                        });
                    }
                });
            } else {
                ...
                realmInstance.write(() => {
                    ...
                    if (saveTo === "workoutPresets") {
                        ...
                        newWorkout = realmInstance.create("WorkoutPresets", { "id": newId, "name": trimmedData.workoutName, "notes": trimmedData.workoutNotes || "" });
                        ...
                        newWorkout = realmInstance.create("PreviousWorkouts", { "id": newId, "name": trimmedData.workoutName, "notes": trimmedData.workoutNotes || "", "date": workoutDate });
                    }
                    ...
                    trimmedData.exercises.forEach((exercise, index) => {
                        ...
                        if (saveTo === "workoutPresets") {
                            realmInstance.create("WorkoutPresetsExercises", { "id": newWorkoutExercisesId, "workoutPresets": newWorkout, "exercises": exerciseObj, "metrics": exercise.duration, "volume": exercise.reps.toString() });
                        } else if (saveTo === "previousWorkouts") {
                            ...
                            realmInstance.create("PreviousWorkoutsExercises", { "id": newPreviousWorkoutExercisesId, "previousWorkouts": newWorkout, "exercises": exerciseObj, "metrics": exercise.duration, "volume": exercise.reps.toString() });
                        }
                    });
                });
            }
    ...
    const updateData = async () => {
        ...
        await saveFormData({ ...allData, "workoutDate": workoutDate.toISOString() });
    };
    ...
    const addSet = (index) => {
        ...
        insert(index + 1, { "name": currentValues.name, "personalBest": currentValues.personalBest || "N/A", "duration": currentValues.duration, "reps": currentValues.reps });
    ...
    const deleteSet = (originalIndex) => {
        ...
        try {
            realmInstance.write(() => {
                const exerciseToDelete = realmInstance.objects("WorkoutPresetsExercises").filtered("workoutPresets.id == $0", parseInt(id))[originalIndex];

                if (exerciseToDelete) { realmInstance.delete(exerciseToDelete); }
            });
        } catch (error) { console.error("Error deleting exercise set from Realm:", error); }
    };
    
    useEffect(() => { getAllWorkedMuscles(); }, [fields, watch("exercises")]);
    ...
    const getPersonalBest = (exerciseName) => {
        const exerciseRecords = realmInstance.objects("PreviousWorkoutsExercises").filtered("exercises.name == $0", exerciseName).sorted("metrics", true);
            ...
            <View className = "items-center m-[5px]">
                {saveTo === "previousWorkouts" && (
                    <View className = "w-11/12">
                        ...
                        {showDatePicker && (
                            <DateTimePicker value = {workoutDate} mode = "date" display = {Platform.OS === "ios" ? "spinner" : "default"} onChange = {(event, selectedDate) => {
                ...
                <Controller control = {control} name = "workoutName" render = {({ "field": { onChange, onBlur, value } }) => { return (
                ...
                <Controller control = {control} name = "workoutNotes" render = {({ "field": { onChange, onBlur, value } }) => { return (
                            ...
                            {group.sets.map((field, index) => { return (
                                <View key = {`${field.fieldId}-${index}`} className = "flex-row w-full">
                                    ...
                                    <View className = "bg-[#f0f0f0] items-center min-h-[100px] flex-1 m-2.5 p-{20px}">
                                        ...
                                        <Controller control = {control} name = {`exercises.${field.originalIndex}.duration`} render = {({ "field": { onChange, onBlur, value } }) => { return (
                                    ...
                                    <View className = "bg-[#f0f0f0] items-center min-h-[100px] flex-1 m-2.5 p-{20px}">
                                        ...
                                        <Controller control = {control} name = {`exercises.${field.originalIndex}.reps`} render = {({ "field": { onChange, onBlur, value } }) => { return (
                                                <TextInput onBlur = {onBlur} onChangeText = {onChange}  value = {value} keyboardType = "numeric" className = "align-middle text-center w-11/12 flex-1 m-2.5 bg-[#DEDEDE]" />
                                            ); }} />
                                    </View>
                                    <TouchableOpacity onPress = {() => { return deleteSet(field.originalIndex); }} className = "ml-2 self-center mt-7">

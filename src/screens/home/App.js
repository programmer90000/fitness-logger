...
// TODO: I have not yet added the isReady handling. This is because I have not yet implemented the Settings screen, which is where the theme is set. Once the Settings screen is implemented, I will add the isReady handling to ensure that the app does not render until the theme is loaded.
...
function App() {
  ...
  return (
    <ScrollView style = {[styles.container, { "backgroundColor": colours.main_background }]} contentContainerStyle = {{ "alignItems": "center" }} > 
        ...
        <Accordion faqData = {faqData} style = "mt-3" />
        <Text className = "mt-5 text-2xl">Benefits Of Using A Fitness Logger</Text>
        <Accordion faqData = {benefitsOfUsingFitnessLogger} style = "mb-10" />
        <Footer />
    </ScrollView>
  );
}
